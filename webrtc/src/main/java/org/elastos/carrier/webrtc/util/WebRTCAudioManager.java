...
package org.elastos.carrier.webrtc.util;
...
/**
 * WebRTCAudioManager manages all audio related parts of the AppRTC demo.
 */
public class WebRTCAudioManager {
  ...
  @Nullable private WebRTCProximitySensor proximitySensor;
  ...
  private void onProximitySensorChangedState() {
    ...
    if (audioDevices.size() == 2 && audioDevices.contains(WebRTCAudioManager.AudioDevice.EARPIECE)
        && audioDevices.contains(WebRTCAudioManager.AudioDevice.SPEAKER_PHONE)) {
      if (proximitySensor.sensorReportsNearState()) {
        // Sensor reports that a "handset is being held up to a person's ear",
        // or "something is covering the light sensor".
        setAudioDeviceInternal(WebRTCAudioManager.AudioDevice.EARPIECE);
      } else {
  ...
  private class WiredHeadsetReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      Log.d(TAG, "WiredHeadsetReceiver.onReceive" + WebRTCUtils.getThreadInfo() + ": "
  ...
  /** Construction. */
  public static WebRTCAudioManager create(Context context) {
    return new WebRTCAudioManager(context);
  }

  private WebRTCAudioManager(Context context) {
    Log.d(TAG, "ctor");
    ...
    proximitySensor = WebRTCProximitySensor.create(context,
  ...
  private void setAudioDeviceInternal(AudioDevice device) {
    ...
    WebRTCUtils.assertIsTrue(audioDevices.contains(device));
  ...
  public void updateAudioDeviceState() {
    ...
    if (bluetoothManager.getState() == WebRTCBluetoothManager.State.HEADSET_UNAVAILABLE
    ...
    } else if (selectedAudioDevice != userSelectedAudioDevice && audioDevices.contains(userSelectedAudioDevice)) {
      // if user select device, change to user selected device.
